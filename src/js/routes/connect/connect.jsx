...
import {Connection, Favorite} from '../../models'
...
export const Connect = React.createClass({
  ...
  componentDidMount() {
    Favorite.on('fav', this.onSelectFavorite)
  },
  componentWillUnmount() {
    Favorite.off('fav', this.onSelectFavorite)
  },
  /*
   * Event Handlers
   */
  onNameChange(e, n) {
    const name = n || n === '' ? n : e.target.value
    if (!name) {
      this.setState({
  ...
  save (e) {
    e.preventDefault();
    const name = this.onNameChange(null, React.findDOMNode(this.refs.name).value)
    if (!name) {
      return
    }
    if (this.state.favorite) {
      Favorite.update(this.state.favorite.id, this.getValues()).then(favorite => {
      ...
      Favorite.create(options).then(favorite => {
        this.setState({ favorite })
      })
    }
  },
  testConnection () {
    const connection = Connection.createInstance(this.getValues())
  ...
  connect (e) {
    e.preventDefault()
    e.stopPropagation()
    let connection = new Connection(this.getValues())
      ...
      <div id="connectPage">
        <div className="row">
          <div className="large-2 medium-3 columns">
            <Favorites activeFav={this.state.favorite}/>
          </div>
